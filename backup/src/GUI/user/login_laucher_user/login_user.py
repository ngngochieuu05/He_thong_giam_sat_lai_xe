...
class UserUI:
    ...
    def _handle_face_register(self, txt_name, txt_phone, txt_username, txt_password, txt_password_confirm):
        ...
        # CRITICAL FIX: Thread-safe lock cho processing flag
        import threading
        ...
        def on_auto_capture(frame: 'np.ndarray', arcface_model):
            """Callback khi tự động chụp ảnh - THREAD-SAFE with lock"""
            nonlocal processing, processing_lock, camera
            
            # CRITICAL FIX: Atomic check-and-set với lock
            with processing_lock:
                if processing:
                    print("[CALLBACK] processing=True, ignoring callback")
                    return
                processing = True
            ...
            def process_face_registration():
                ...
                try:
                    ...
                    # FIX: Normalize frame nếu resolution quá lớn - resize xuống để detection dễ hơn
                    # Frame từ camera: 480x640 có thể quá lớn/sáng/tối
                    # Preprocesses: convert sang grayscale để test detection
                    test_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
                    test_gray = clahe.apply(test_gray)
                    
                    print(f"   ├─ Frame stats: min={test_gray.min()}, max={test_gray.max()}, mean={test_gray.mean():.1f}")
                    ...
                    if success:
                        ...
                        # Thread-safe reset processing flag
                        with processing_lock:
                            processing = False
