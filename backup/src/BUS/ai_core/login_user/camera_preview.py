...
class LiveCameraPreview:
                ...
                if self.face_detected:
                    with self._capture_lock:
                        ...
                        if not self.auto_captured and self.on_auto_capture:
                            print("[CAMERA] 🎯 Setting auto_captured to True")
                    ...
                    if self.auto_captured and self.on_auto_capture:
                        print(f"[CAMERA] 📸 Calling on_auto_capture with frame shape: {frame.shape}")
    ...
    def _detect_face_in_oval(self, frame: np.ndarray) -> bool:
        ...
        try:
            ...
            if len(faces) > 0:
                print(f"🎯 [OVAL_DETECT] Found {len(faces)} face(s)")
                ...
                if distance <= 1.5:
                    print(f"✅ [OVAL_DETECT] Face detected in oval! Distance: {distance:.3f}")
                    return True
                else:
                    print(f"⚠️ [OVAL_DETECT] Face found but outside oval. Distance: {distance:.3f} (threshold: 1.5)")
